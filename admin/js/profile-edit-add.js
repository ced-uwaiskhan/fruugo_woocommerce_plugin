jQuery( document ).ready(
	function() {
		jQuery( document ).find( '#_umb_fruugo_category' ).select();

		jQuery( '.ced_fruugo_profile_metakeys_li' ).on(
			'mouseover',
			function(){
				jQuery( '.ced_fruugo_sel_metakey' ).css( 'box-shadow', "0 0 1em #888888" );
			}
		);
		jQuery( '.ced_fruugo_profile_basic_detail_li' ).on(
			'mouseover',
			function(){
				jQuery( '.ced_fruugo_basic_set' ).css( 'box-shadow', "0 0 1em #888888" );
			}
		);
		jQuery( '.ced_fruugo_profile_required_field_li' ).on(
			'mouseover',
			function(){
				jQuery( '.ced_fruugo_req_field' ).css( 'box-shadow', "0 0 1em #888888" );
			}
		);
		jQuery( '.ced_fruugo_profile_category_li' ).on(
			'mouseover',
			function(){
		...
		jQuery( '.ced_fruugo_save_profile_button' ).on(
			'click',
			function(e){
				e.preventDefault();
				var f      = 0;
		...
		function updateMetaKeysInDBForProfile( metaKey , actionToDo ) {
			...
			jQuery.ajax({
				...
				data : {
					...
					nonce:ced_fruugo_profile_edit_add_script_AJAX.nonce,
					metaKey : metaKey,
		...
		jQuery( '.ced_fruugo_profile_timeline_ul li' ).on(
			'click',
			function(){
		...
		jQuery( document.body ).on(
			'click',
			'input:checkbox[class=ced_fruugo_add_del_meta_keys]',
			function() {
...
function updateMetaKeysInDBForProfile( metaKey , actionToDo ) {
	jQuery( "#ced_fruugo_marketplace_loader" ).show();
	//alert(ced_fruugo_profile_edit_add_script_AJAX.nonce);
	jQuery.ajax(
		{
			url : ced_fruugo_profile_edit_add_script_AJAX.ajax_url,
			type : 'post',
...
jQuery( document ).ready(
	function() {
		...
		jQuery( document.body ).on(
			'click',
			'div.ced_fruugo_tabbed_head_wrapper ul li',
			function(){
...
jQuery( document ).ready(
	function() {
		...
		jQuery( document.body ).on(
			'click',
			'table#ced_fruugo_products_matched td',
			function(){
